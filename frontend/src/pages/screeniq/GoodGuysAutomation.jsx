...
function BeforeRule() {
  ...
  const [ruleType, setRuleType] = useState('trusted_entity')
  ...
  return (
    <div className="space-y-5">
      <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-4">
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1.5">Good guys rule type</label>
          <div className="flex flex-wrap gap-3">
            {[
              { value: 'trusted_entity', label: 'Trusted entity' },
              { value: 'internal', label: 'Internal / intercompany' },
              { value: 'whitelisted_counterparty', label: 'Whitelisted counterparty' },
              { value: 'known_correspondent', label: 'Known correspondent' },
            ].map(opt => (
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1.5">
            Describe the good guys rule
          </label>
          <textarea
            value={description}
            onChange={e => setDescription(e.target.value)}
            placeholder="e.g. Suppress alerts for transactions between known internal subsidiaries where both entities appear on our approved internal entity list, regardless of transaction amount..."
            rows={5}
            className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 resize-none focus:outline-none focus:ring-1 focus:ring-green-400"
          />
        </div>
      ...
      {result && (
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            <p className="text-xs font-semibold text-green-700 uppercase tracking-wide">Generated rule</p>
            <div className="flex gap-2">
              <button
                onClick={() => downloadText('generated-good-guys-rule.txt', result.rule)}
...
function DuringRule() {
  ...
  const handleInterpret = async () => {
    ...
    await new Promise(r => setTimeout(r, 900))
    setResult({
      summary: `Parsed good guys rule content (${text.split('\n').length} lines). Detailed interpretation will be available once rule format is configured.`,
      rules: [
        { id: 'GG-001', name: 'Example good guys rule', entityType: 'Trusted entity', trigger: 'Pending configuration', fields: ['entity_name', 'account_id'], action: 'suppress' },
      ],
    })
  ...
  const handleExportSummary = () => {
    if (!result) return
    const lines = [
      'GOOD GUYS RULE INTERPRETATION SUMMARY',
      '======================================',
      ...
      '--------------',
      ...result.rules.map(r =>
        `ID: ${r.id} | Name: ${r.name} | Entity Type: ${r.entityType} | Trigger: ${r.trigger} | Fields: ${r.fields.join(', ')} | Action: ${r.action}`
      ),
    ]
    downloadText('good-guys-interpretation-summary.txt', lines.join('\n'))
  }
  ...
  return (
    <div className="space-y-5">
      <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-4">
            ...
            {[['paste', 'Paste rule text'], ['upload', 'Upload file']].map(([val, label]) => (
              <button
                ...
                className={`text-sm px-4 py-1.5 rounded-lg font-medium transition-colors ${
                  inputMode === val ? 'bg-green-600 text-white' : 'text-slate-600 hover:bg-slate-100'
                }`}
              >
                {label}
              </button>
        ...
        {inputMode === 'paste' ? (
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1.5">Good guys rule content</label>
            <textarea
              value={pasteText}
              onChange={e => setPasteText(e.target.value)}
              placeholder="Paste one or more good guys rules here…"
              rows={8}
              className="w-full text-sm font-mono border border-slate-200 rounded-lg px-3 py-2.5 resize-none focus:outline-none focus:ring-1 focus:ring-green-400"
            />
          </div>
        ) : (
          <div className="space-y-3">
            <div
              ...
              className="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center cursor-pointer hover:border-green-300 hover:bg-green-50/30 transition-colors"
            >
              ...
              {fileName ? (
                ...
                <>
                  <p className="text-sm font-medium text-slate-600">Click to upload good guys rule file</p>
      ...
      {result && (
        <div className="space-y-4">
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
              <p className="text-xs font-semibold text-green-700 uppercase tracking-wide">Summary</p>
              <button
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-100 bg-slate-50">
              <p className="text-xs font-semibold text-green-700 uppercase tracking-wide">Good guys rule inventory</p>
            </div>
            <div className="divide-y divide-slate-100">
              {result.rules.map((rule, i) => (
                <div key={i} className="px-5 py-4 grid grid-cols-5 gap-4 text-sm">
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-slate-600">{rule.entityType}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-400 mb-0.5">Trigger</p>
                    <p className="text-slate-500">{rule.trigger}</p>
                  </div>
                  <div>
...
function AfterRule() {
  ...
  const handleGenerate = async () => {
    ...
    setLog({
      ...
      entries: [
        {
          id: 'GG-001',
          name: 'Example good guys rule',
          type: 'Trusted entity',
          entityClass: 'Internal subsidiary',
          trigger: 'Pending configuration',
          fieldsRead: ['entity_name', 'account_id'],
  ...
  const handleExportLog = () => {
    if (!log) return
    downloadCSV(
      `good-guys-log-${new Date().toISOString().slice(0, 10)}.csv`,
      log.entries.map(r => ({
        'Rule ID': r.id,
        'Name': r.name,
        'Type': r.type,
        'Entity Class': r.entityClass,
        'Trigger': r.trigger,
      ...
      ['Rule ID', 'Name', 'Type', 'Entity Class', 'Trigger', 'Fields Read', 'Fields Written', 'Action', 'Notes']
    )
  }
    ...
    <div className="space-y-5">
        ...
        {inputMode === 'upload' ? (
          ...
          <div>
            ...
            <textarea
              value={pasteText}
              onChange={e => setPasteText(e.target.value)}
              placeholder="Paste good guys rule file content here…"
              rows={8}
              className="w-full text-sm font-mono border border-slate-200 rounded-lg px-3 py-2.5 resize-none focus:outline-none focus:ring-1 focus:ring-green-400"
            />
          </div>
      ...
      {log && (
        <div className="space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
              <p className="text-xs font-semibold text-green-700 uppercase tracking-wide">Good guys rule log</p>
              <button
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-xs text-slate-500 uppercase tracking-wide">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left">Entity class</th>
              ...
              <tbody>
                {log.entries.map((row, i) => (
                  <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                    ...
                    <td className="px-4 py-3 text-slate-500">{row.entityClass}</td>
...
export default function GoodGuysAutomation() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
      <div>
        ...
        <h1 className="text-2xl font-bold text-slate-900">Good Guys Manager</h1>
        <p className="text-sm text-slate-500 mt-1">
          Create, interpret, and document trusted entity and whitelist rules using AI-assisted tooling.
        </p>
      </div>
